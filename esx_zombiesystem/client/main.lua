    ...
    while true do
        ...
        if #entitys < Config.SpawnZombie then
            ...
            repeat
                ...
                for _, player in pairs(players) do
                    ...
                    if (posX > playerX - Config.MinSpawnDistance and posX < playerX + Config.MinSpawnDistance) or
                       (posY > playerY - Config.MinSpawnDistance and posY < playerY + Config.MinSpawnDistance) then
                        canSpawn = false
                        break
                    else
                        canSpawn = true
        ...
        for i, entity in pairs(entitys) do
            if not DoesEntityExist(entity) then
                ...
                if pedX < playerX - Config.DespawnDistance or pedX > playerX + Config.DespawnDistance or
                   pedY < playerY - Config.DespawnDistance or pedY > playerY + Config.DespawnDistance then
			...
			for i, entity in pairs(entitys) do
				if IsPedDeadOrDying(entity, 1) == 1 then
					...
					if sourceOfDeath == PlayerPedId() then
						...
						local randomLootType = math.random(1, 3)

						if randomLootType == 1 then
						...
						SetModelAsNoLongerNeeded(model)
						table.remove(entitys, i)
					elseif IsEntityAVehicle(sourceOfDeath) then
						-- Prevent loot drop if killed by a vehicle
						local model = GetEntityModel(entity)
						SetEntityAsNoLongerNeeded(entity)
						SetModelAsNoLongerNeeded(model)
						table.remove(entitys, i)
